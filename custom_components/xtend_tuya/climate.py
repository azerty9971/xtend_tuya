...
MERGED_HVAC_TO_HA: dict[str, HVACMode] = append_dictionnaries(XT_HVAC_TO_HA, TuyaClimateHVACToHA)
...
XT_CLIMATE_SET_TEMPERATURE_DPCODES: tuple[XTDPCode, ...] = append_tuples(XT_CLIMATE_SET_CELSIUS_TEMPERATURE_DPCODES, XT_CLIMATE_SET_FAHRENHEIT_TEMPERATURE_DPCODES)
...
XT_CLIMATE_SWING_MODE_DPCODES: tuple[XTDPCode, ...] = append_tuples(append_tuples(XT_CLIMATE_SWING_MODE_ON_DPCODES, XT_CLIMATE_SWING_MODE_HORIZONTAL_DPCODES), XT_CLIMATE_SWING_MODE_VERTICAL_DPCODES)
...
class XTClimateEntity(XTEntity, TuyaClimateEntity):
    ...
    def __init__(
        ...
        if any(
            ...
            for dpcode in XT_CLIMATE_CURRENT_TEMPERATURE_DPCODES
        ) or any(
        ...
        for dpcode in XT_CLIMATE_TEMPERATURE_UNIT_DPCODES:
            if dpcode in device.status:
                self.control_dp_codes[XTClimateEntity.ControlDPCode.TEMPERATURE_UNIT] = dpcode
        ...
        if fahrenheit_type and (
            ...
            self.control_dp_codes[XTClimateEntity.ControlDPCode.CURRENT_TEMPERATURE] = XTDPCode(fahrenheit_type.dpcode)
            self._attr_temperature_unit = UnitOfTemperature.FAHRENHEIT
            self._current_temperature = fahrenheit_type
        elif celsius_type:
            self.control_dp_codes[XTClimateEntity.ControlDPCode.CURRENT_TEMPERATURE] = XTDPCode(celsius_type.dpcode)
            self._attr_temperature_unit = UnitOfTemperature.CELSIUS
            self._current_temperature = celsius_type

        # Figure out setting temperature, use preferred unit or what is available
        celsius_type = self.find_dpcode(
            XT_CLIMATE_SET_CELSIUS_TEMPERATURE_DPCODES, dptype=TuyaDPType.INTEGER, prefer_function=True, only_function=True
        )
        fahrenheit_type = self.find_dpcode(
            XT_CLIMATE_SET_FAHRENHEIT_TEMPERATURE_DPCODES, dptype=TuyaDPType.INTEGER, prefer_function=True, only_function=True
        )
        if fahrenheit_type and (
            prefered_temperature_unit == UnitOfTemperature.FAHRENHEIT
            or (
        ...
        if self._set_temperature:
            self.control_dp_codes[XTClimateEntity.ControlDPCode.SET_TEMPERATURE] = XTDPCode(self._set_temperature.dpcode)
        ...
        if enum_type := self.find_dpcode(
            XT_CLIMATE_MODE_DPCODES, dptype=TuyaDPType.ENUM, prefer_function=True, only_function=True
        ):
            self.control_dp_codes[XTClimateEntity.ControlDPCode.HVAC_MODE] = XTDPCode(enum_type.dpcode)
        ...
        elif self.find_dpcode(XT_CLIMATE_SWITCH_DPCODES, prefer_function=True, only_function=True):
        ...
        if int_type := self.find_dpcode(
            XT_CLIMATE_SET_HUMIDITY_DPCODES, dptype=TuyaDPType.INTEGER, prefer_function=True, only_function=True
        ):
            self.control_dp_codes[XTClimateEntity.ControlDPCode.SET_HUMIDITY] = XTDPCode(int_type.dpcode)
        ...
        if self._current_humidity:
            self.control_dp_codes[XTClimateEntity.ControlDPCode.CURRENT_HUMIDITY] = XTDPCode(self._current_humidity.dpcode)
        ...
        if enum_type := self.find_dpcode(
            ...
            self.control_dp_codes[XTClimateEntity.ControlDPCode.FAN_SPEED] = XTDPCode(enum_type.dpcode)
        ...
        if self.find_dpcode(
            ...
            if dpcode := self.find_dpcode(XT_CLIMATE_SWING_MODE_ON_DPCODES, prefer_function=True, only_function=True):
            ...
            if dpcode := self.find_dpcode(XT_CLIMATE_SWING_MODE_HORIZONTAL_DPCODES, prefer_function=True, only_function=True):
                self.control_dp_codes[XTClimateEntity.ControlDPCode.SWING_MODE_HORIZONTAL] = XTDPCode(dpcode)
                self._attr_swing_modes.append(SWING_HORIZONTAL)

            if dpcode := self.find_dpcode(XT_CLIMATE_SWING_MODE_VERTICAL_DPCODES, prefer_function=True, only_function=True):
                self.control_dp_codes[XTClimateEntity.ControlDPCode.SWING_MODE_VERTICAL] = XTDPCode(dpcode)
        ...
        for switch_dpcode in XT_CLIMATE_SWITCH_DPCODES:
            if switch_dpcode in self.device.function:
                ...
                self.control_dp_codes[XTClimateEntity.ControlDPCode.SWITCH_ON] = switch_dpcode
                break
    ...
    def get_swing_mode_on_value(self, bool_swing: bool) -> bool | str:
        if self.swing_mode_enum_type is not None:
            for allowed_value in self.swing_mode_enum_type.range:
                if XT_CLIMATE_SWING_MODE_ENUM_VALUE_MAPPING.get(allowed_value.lower()) is bool_swing:
                    return allowed_value
        return bool_swing
    
    def is_swing_mode_on(self) -> bool:
        swing_mode_status_value = self.device.status.get(self.control_dp_codes.get(XTClimateEntity.ControlDPCode.SWING_MODE_ON, ""))
        if self.swing_mode_enum_type is not None:
            return XT_CLIMATE_SWING_MODE_ENUM_VALUE_MAPPING.get(str(swing_mode_status_value).lower(), bool(swing_mode_status_value))
    ...
    def set_swing_mode(self, swing_mode: str) -> None:
        ...
        if dpcode_swing := self.control_dp_codes.get(XTClimateEntity.ControlDPCode.SWING_MODE_ON):
            commands.append(
                {"code": dpcode_swing, "value": self.get_swing_mode_on_value(swing_mode != SWING_OFF)}
            )
        if dpcode_swing_horizontal := self.control_dp_codes.get(XTClimateEntity.ControlDPCode.SWING_MODE_HORIZONTAL):
            commands.append(
                {"code": dpcode_swing_horizontal, "value": swing_mode in (SWING_BOTH, SWING_HORIZONTAL)}
            )
        if dpcode_swing_vertical := self.control_dp_codes.get(XTClimateEntity.ControlDPCode.SWING_MODE_VERTICAL):
            commands.append(
    ...
    def fan_mode(self) -> str | None:
        """Return fan mode."""
        return self.device.status.get(self.control_dp_codes.get(XTClimateEntity.ControlDPCode.FAN_SPEED, ""))

    @property
    def swing_mode(self) -> str:
        """Return swing mode."""
        horizontal = self.device.status.get(self.control_dp_codes.get(XTClimateEntity.ControlDPCode.SWING_MODE_HORIZONTAL, ""))
        vertical = self.device.status.get(self.control_dp_codes.get(XTClimateEntity.ControlDPCode.SWING_MODE_VERTICAL, ""))
