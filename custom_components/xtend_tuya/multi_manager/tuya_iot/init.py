...
class XTTuyaIOTDeviceManagerInterface(XTDeviceManagerInterface):
    ...
    def update_device_cache(self):
        ...
        for device in self.iot_account.device_manager.device_map.values():
            self.multi_manager.device_watcher.report_message(device.id, f"Update device cache {MESSAGE_SOURCE_TUYA_IOT}: status: {device.status}")
            self.multi_manager.device_watcher.report_message(device.id, f"Update device cache {MESSAGE_SOURCE_TUYA_IOT}: status_range: {device.status_range}")
            self.multi_manager.device_watcher.report_message(device.id, f"Update device cache {MESSAGE_SOURCE_TUYA_IOT}: function: {device.function}")
            self.multi_manager.device_watcher.report_message(device.id, f"Update device cache {MESSAGE_SOURCE_TUYA_IOT}: local_strategy: {device.local_strategy}")
