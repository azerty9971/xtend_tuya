...
class XTIOTOpenAPI:
    ...
    def __refresh_access_token_if_need(self, path: str):
        LOGGER.debug(f"[{self.class_id}]Calling __refresh_access_token_if_need")
        ...
        if expired_time - 60 * 1000 > now:  # 1min
            LOGGER.debug(f"[{self.class_id}]__refresh_access_token_if_need token is not old enough ({expired_time - 60 * 1000} > {now})")
            return
        ...
        LOGGER.debug(f"[{self.class_id}]__refresh_access_token_if_need response: {response}")
    ...
    def connect(
        ...
        LOGGER.debug(f"[{self.class_id}]Calling connect")
        if self.auth_type == AuthType.CUSTOM:
            response = self.post(
    ...
    def reconnect(self) -> bool:
        LOGGER.debug(f"[{self.class_id}]Calling reconnect (connecting: {self.connecting})")
        if (
            not self.connecting
            and self.__username 
            and self.__password
    ...
    def is_connect(self) -> bool:
        ...
        LOGGER.debug(f"[{self.class_id}]is_connect = {ret_val}")
        return ret_val
    ...
    def __request(
        ...
        LOGGER.debug(f"[{self.class_id}]Requesting: {method} {path} (first_pass={first_pass})")
        ...
        for _ in range(10):
            try:
                ...
                LOGGER.debug(f"[{self.class_id}]Exception in request, waiting for 2 seconds and retrying")
                time.sleep(2)

        if response.ok is False:
            LOGGER.error(
                f"[{self.class_id}]Response error: code={response.status_code}, body={response.body}"
            )
            return None
        ...
        LOGGER.debug(
            f"[{self.class_id}]Response: {json.dumps(result, ensure_ascii=False, indent=2)}"
        )
