...
class XTIOTDeviceManager(TuyaDeviceManager):
    ...
    def send_lock_unlock_command(
        ...
        self.multi_manager.device_watcher.report_message(device_id, f"Sending lock/unlock command open: {open}")
    ...
    def get_supported_unlock_types(self, device_id: str) -> list[str]:
        ...
        self.multi_manager.device_watcher.report_message(device_id, f"API remote unlock types: {remote_unlock_types}")
    ...
    def get_door_lock_password_ticket(self, device_id: str) -> str | None:
        ...
        self.multi_manager.device_watcher.report_message(device_id, f"API remote unlock ticket: {ticket}")
    ...
    def call_door_operate(self, device_id: str, open: str) -> bool:
        if ticket_id := self.get_door_lock_password_ticket(device_id):
            ...
            self.multi_manager.device_watcher.report_message(device_id, f"API call_door_operate result: {lock_operation}")
    ...
    def call_door_open(self, device_id: str) -> bool:
        if ticket_id := self.get_door_lock_password_ticket(device_id):
            ...
            self.multi_manager.device_watcher.report_message(device_id, f"API call_door_open result: {lock_operation}")
            if lock_operation.get("success", False):
                return True
        return False