    ...
    def copy_statuses_to_tuya(self, device: XTDevice) -> bool:
        ...
        if other_manager := self.get_overriden_device_manager():
            if device.id in other_manager.device_map:
                self.multi_manager.device_watcher.report_message(f"BEFORE copy_statuses_to_tuya: {other_manager.device_map[device.id].status}")
                ...
                self.multi_manager.device_watcher.report_message(f"AFTER copy_statuses_to_tuya: {other_manager.device_map[device.id].status}")
