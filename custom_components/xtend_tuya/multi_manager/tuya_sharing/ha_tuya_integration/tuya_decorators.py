...
def decorate_tuya_manager(tuya_manager: TuyaManager, ha_tuya_integration_config_manager: XTHATuyaIntegrationConfigEntryManager) -> list[XTDecorator]:
    ...
    decorator, tuya_manager.refresh_mq  = XTDecorator.get_decorator(tuya_manager, ha_tuya_integration_config_manager.on_tuya_refresh_mq, "refresh_mq")
    return_list.append(decorator)

    decorator, tuya_manager.on_message  = XTDecorator.get_decorator(tuya_manager, None, "on_message")
    return_list.append(decorator)
...
def decorate_tuya_integration(ha_tuya_integration_config_manager: XTHATuyaIntegrationConfigEntryManager) -> list[XTDecorator]:
    ...
    decorator, tuya_integration.async_setup_entry  = XTDecorator.get_async_decorator(tuya_integration, ha_tuya_integration_config_manager.on_tuya_setup_entry, "async_setup_entry")
    return_list.append(decorator)

    decorator, tuya_integration.async_unload_entry  = XTDecorator.get_async_decorator(tuya_integration, ha_tuya_integration_config_manager.on_tuya_unload_entry, "async_unload_entry")
    return_list.append(decorator)

    decorator, tuya_integration.async_remove_entry  = XTDecorator.get_async_decorator(tuya_integration, ha_tuya_integration_config_manager.on_tuya_remove_entry, "async_remove_entry")
    return_list.append(decorator)
    return return_list
